...
%% Load dataset.
% choose from: face, flag, halfring, iris, moon, ring, spiral, wave, wine
dataName = 'face'; 
...
for k = 1:maxK-1
    % original Dunn's index (Dunn, 1973)
    DNo(k) = indexDN(data,labels(:,k),'euclidean');
...
figure();
subplot(3,1,1); plot(2:maxK, DNo,'.-k');hold on; plot(ind_o+1,val_o,'sr');
title('Original Dunn''s index');
xlabel('number of clusters'); ylabel('index value');

subplot(3,1,2); plot(2:maxK, DNg,'.-k');hold on; plot(ind_g+1,val_g,'sr');
title('Generalized Dunn''s index');
xlabel('number of clusters'); ylabel('index value');

subplot(3,1,3); plot(2:maxK, DNs,'.-k');hold on; plot(ind_s+1,val_s,'sr');
