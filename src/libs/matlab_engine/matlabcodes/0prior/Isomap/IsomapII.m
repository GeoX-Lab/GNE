     ...
     if n_fcn == 'k'
         ...
         for i=1:N
             ...
             if ((verbose == 1) & (rem(i,50) == 0)) 
                 disp([' Iteration: ' num2str(i) '     Estimated time to completion: ' ' minutes']); tic; 
                 %num2str((N-i)*toc/60/50)
             end
         end
     elseif n_fcn == 'epsilon'
...
if ((mode==1) & (use_dijk == 0))
     ...
     for k=1:N
         ...
         if ((verbose == 1) & (rem(k,20) == 0)) 
              disp([' Iteration: ' num2str(k) '     Estimated time to completion: ' ' minutes']); 
              %num2str((N-i)*toc/i/60)
         end
     end
else
